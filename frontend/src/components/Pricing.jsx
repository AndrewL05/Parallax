...
const PricingSection = () => {
  ...
  const handlePremiumClick = async () => {
    try {
      ...
      const checkout = await stripeService.createCheckoutSession('premium_monthly', token);
  ...
  const handleProClick = async () => {
    try {
      const token = isSignedIn ? await getToken() : null;
      const checkout = await stripeService.createCheckoutSession('premium_yearly', token);
      if (checkout.checkout_url) {
        window.location.href = checkout.checkout_url;
      }
    } catch (error) {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {plans.map((plan, index) => (
            <motion.div
              ...
              className={`bg-white rounded-2xl shadow-xl p-8 relative ${plan.popular ? 'border-4 border-purple-500 scale-105' : ''}`}
            >
              {plan.popular && (
